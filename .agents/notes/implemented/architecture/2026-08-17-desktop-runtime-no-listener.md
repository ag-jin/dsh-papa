...
The runtime package verifies unary dispatch, receipts, cancellation, malformed bridge input, per-window stream abort, runtime invariants, real Cordis plugin mounting and disposal, package invariants, and the desktop patch resolver closure. The desktop app verifies protocol containment and catalog authorization, boot-manifest injection, preload allowlisting, window security options and CSP, fixed IPC registration, listener-free patch boot, and normalized desktop-local state.
