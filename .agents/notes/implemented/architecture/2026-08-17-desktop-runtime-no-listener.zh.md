...
运行时包验证一元分发、回执、取消、格式错误的 bridge 输入、按窗口的流终止、运行时 invariant、真实 Cordis 插件挂载与释放、包 invariant 和桌面补丁解析器闭包。桌面应用验证 protocol containment 和 catalog authorization、boot-manifest injection、preload allowlisting、window security option 和 CSP、固定 IPC registration、listener-free patch boot，以及归一化的 desktop-local state。
