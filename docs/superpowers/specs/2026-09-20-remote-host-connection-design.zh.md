...
### 连接使用 SSH 端口与凭据认证，而非 ssh config 别名

一条远端主机记录直接携带 SSH 端点：主机、端口、用户与密码。它不依赖操作者的 `~/.ssh/config`，也不依赖预置密钥，因此仅在本应用里配置过的主机，即可从本应用访问到。

有两种载体可以满足这一点，且两者都针对一台专门搭建、确实接受密码认证的 SSH 服务器做了端到端实测：

| 载体 | 密码认证 | 端口转发 | 经隧道加载完整 GUI |
|---|---|---|---|
| 系统 `ssh` + `SSH_ASKPASS` | 认证通过 | 已建立 | 200，27,660 字节 |
| `ssh2`（纯 JS 库） | 认证通过 | 已建立 | 200，27,660 字节 |

选择系统 `ssh` 载体，理由有三。

**主机密钥校验。** `ssh2` 的 `hostVerifier` 默认自动接受未知主机密钥，采用它就意味着要自己编写并维护 known-hosts 校验——那是真实的、与安全相关的自有代码。系统 `ssh` 继承 `StrictHostKeyChecking` 与 known-hosts 处理，而这正是 `packages/ssh/ssh/src/index.ts` 已经依赖的信任模型。

**与仓库的契合度。** 现有的 SSH 集成就是 spawn 系统 `ssh`，所选载体与之一致。`ssh2` 是 CommonJS，与仓库仅 ESM 的源码启动约定冲突，且其 registry 元数据**不含 `license` 字段**，采用前需要做供应链审查。

**替代方案的代价。** 这并不意味着两者可以互换：纯 JS 那个在 Windows 上更合适，因为在 Windows 上 spawn 一个带 `SSH_ASKPASS` 的 POSIX `ssh` 不可移植。现在推迟它没有代价，因为第一阶段面向的是 macOS 上的 Web GUI。

密码经 `SSH_ASKPASS` 传给系统 `ssh`：一个打印所存密码的辅助程序，配合 `SSH_ASKPASS_REQUIRE=force`，使其在无终端时也被咨询。转发使用 `-N -L <本地>:127.0.0.1:<远端> -p <ssh端口>`，并带 `ExitOnForwardFailure=yes`，使转发失败时连接即失败，而不会留下一个什么都不应答的监听者。

### 远端主机记录

每条记录携带：显示别名、SSH 主机、SSH 端口、SSH 用户、远端 `dsh web` 端口、固定的本地隧道端口，以及缓存的浏览器 cookie。

SSH 密码经 `ctx.credentials` 以 `grant` 记录存储，与 `packages/client/connection/src/browser-auth.ts` 存储浏览器会话签名密钥的方式完全相同。这正是无人值守重连得以成立的原因：隧道所有者每次重连时重新读取密码，而不是弹出提示。代价是明确的——`$DSH_HOME/.credentials.yaml` 此后同时持有远端浏览器会话 cookie 与 SSH 密码，因此它成为一个高价值文件，必须保持仅属主可读。

远端启动令牌不是持久记录：它在远端按进程生成，远端每次重启都会变化。浏览器 cookie 是持久的，在同一 authority 上可跨远端重启存续，直到其配置的有效期结束（默认 30 天）。
...
一份已配置远端主机的持久列表，存放于 `$DSH_HOME` 下（依据用户决定：它随 DSH home 走，而非随工作区走）。其记录字段与密码的存储方式见上文「远端主机记录」一节。
...
1. 用户在侧边栏选中一个远端主机。
2. 注册表解析其记录；隧道所有者从凭据中读取该记录的密码，并在该记录固定的本地端口上建立 SSH 隧道。
...
| 失败 | 必须给出的报告 |
|---|---|
| SSH 认证被拒 | 指出主机、端口与用户，并说明密码被拒 |
| SSH 主机密钥未知或已变更 | 指出该主机，并说明记录的密钥已不匹配，由操作者决定是否重新信任 |
| SSH 端口不可达 | 指出主机与端口，并说明连接被拒绝或超时 |
| 远端没有 `dsh` 或不在 `PATH` 上 | 说明远端没有可运行的 `dsh` |
| 远端 `dsh web` 端口被其他进程占用 | 指出该端口，并说明远端已有服务在跑 |
| 远端重启后令牌过期 | 提供从远端重新读取令牌并重试 |
| 会话中途隧道断开 | 使用所存密码自动重连；若 cookie 仍有效，则无需用户操作 |
...
| SSH 认证 | 密码经 SSH 传输，通过 `SSH_ASKPASS` 交给系统 `ssh`；主机密钥校验保持开启（`StrictHostKeyChecking`），被替换的主机会被拒绝 |
| SSH 密码落盘 | 以 `grant` 凭据记录形式存于 `$DSH_HOME` 下，仅属主可读；它使无人值守重连成为可能，也正是该文件成为高价值目标的原因 |
...
- **单元：** 隧道所有者的 argv 构造、`SSH_ASKPASS` 交接、就绪探测与拆除；注册表记录的往返、凭据记录存储与校验；隧道端口预留与冲突上报。
- **集成：** 一台确实接受密码认证的真实 SSH 服务器，配合在本地 `dsh web` 前置的一条真实隧道，证明密码认证成功、令牌交换铸出绑定 authority 的 cookie，且经隧道携带 cookie 的 `/api` 调用通过栅栏、不带 cookie 的调用得到 401。这与上文记录的实测一致。
...
- **纯密码认证只接受密码。** 所选载体关闭了公钥认证，使记录的密码成为唯一凭据。只允许密钥登录的主机在本设计加入密钥记录之前无法访问。
- **密码这条路径是 POSIX 专属的。** `SSH_ASKPASS` 加 spawn `ssh` 是 macOS 与 Linux 的认证方式；Windows 在这套形态下两者都没有。`ssh2` 载体是那里的答案，它被推迟而非否决。
