...
flowchart LR
  ...
  svc_remoteHosts["ctx.remoteHosts<br/>Host remote-Harness connection controller"]
...
| `ctx.remoteHosts` | `core` | [`remote-hosts`](../packages/remote/remote-hosts) | - | - | - | 拥有已配置的远端主机、存于凭据库中的 SSH 密码与 Web 访问令牌，以及每台主机一条仅回环隧道，经由生成的 Remote 命名空间。 |
